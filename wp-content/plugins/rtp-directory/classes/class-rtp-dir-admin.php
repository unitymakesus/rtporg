 ...
 * @author Jeffikus
 */
final class RTP_Dir_Admin {
	/**
	 * RTP_Dir_Admin The single instance of RTP_Dir_Admin.
	 * @var 	object
	 * @access  private
	 * @since 	1.0.0
	 */
	...
	/**
	 * Main RTP_Dir_Admin Instance
	 *
	 * Ensures only one instance of RTP_Dir_Admin is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main RTP_Dir_Admin instance
	 */
	...
	public function settings_screen () {
		global $title;
		$sections = RTP_Dir()->settings->get_settings_sections();
	...
	public function render_settings ( $args ) {
		$token = $args['id'];
		$fields = RTP_Dir()->settings->get_settings_fields( $token );
		...
		if ( 0 < count( $fields ) ) {
			foreach ( $fields as $k => $v ) {
				...
				add_settings_field( $k, $v['name'], array( RTP_Dir()->settings, 'render_field' ), 'starter-plugin-' . $token , $v['section'], $args );
			}
		}
	} // End render_settings()
	...
	public function validate_settings ( $input ) {
		$sections = RTP_Dir()->settings->get_settings_sections();
		$tab = $this->_get_current_tab( $sections );
		return RTP_Dir()->settings->validate_settings( $input, $tab );
